...
<script type="text/javascript">
	...
	$(function() {
		...
		kkpager.generPageHtml({
			...
			//链接尾部
			hrefLatter : '/to_withdrawal_enterprise_true',
			getLink : function(n) {
	...
	<div class="rightcon">
		<div class="right_con">
			...
			<ul class="glb-search">
				<!-- 通过用户的需求查到相关信息 -->
				<form
					action="customerServiceController/to_withdrawal_enterprise_true"
			...
			<div class="container">
				<div class="container">
					<table class="glb-tables" align="center">
						...
						<tbody>
							<c:forEach items="${requestScope.enterpriseCustw }" var="ct">
								<c:if test="${ct.state == 4 }">
									<tr>
										<td>${ct.id }</td>
										<td>${ct.enterprise.enterpriseName }</td>
										<td>${ct.money }</td>
										<td><fmt:formatDate value="${ct.time }" type="both" /></td>
										<td><c:if test="${ct.operation==2 }">企业提现</c:if></td>
										<td>已审核</td>
										<td>已通过审核</td>
									</tr>
								</c:if>
