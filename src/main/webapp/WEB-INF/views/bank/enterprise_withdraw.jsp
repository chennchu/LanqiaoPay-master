...
<script type="text/javascript">
	$(function() {
		...
		$("#Allmoney2").blur(function() {
			var m = $("[name='nameBalance2']").val();
			var liyy = liy;
			// 获取用户输入的提现金额
			var mon = $("[name= winthdrawMoney2]").val();
			// 获取隐藏的userId
			var id = $("[name= userId2]").val();
			var url = "<%=request.getContextPath()%>/BankController/moneyIsTrueOrFalse_enterprise";
			var args = {
				"time" : new Date(),
				"id" : id,
				"mon" : mon,
			...
			$.ajax({
				...
				success : function(result) {
					if (result != false) {
						...
						// 获取生成的利息
						strMoney = result;
						$("#paritiesEnterprise").text(strMoney);
						return true;
					}
				}
			});
		...
		// 点击全部提现之后获取所有用户的账户金额
		$("[name = 'cbAllMoney2']").click(function() {
			// 获取输入框中的金额
			var mm = $("[name= winthdrawMoney2]").val();
			var m = $("[name='nameBalance2']").val();
			if (mm == m) {
				$("[name= 'winthdrawMoney2']").val("0.00");
			} else if (mm == 0) {
				$("[name= 'winthdrawMoney2']").val(m);
			} else if (mm < m) {
				$("[name= 'winthdrawMoney2']").val(m);
			}
		})
		...
		$(":submit").click(function() {
			var id = $("[name=id2]").val();
			var payPassword = $("[name=payPassword2]").val();
			...
			var newPayPassword = $("[name=newPayPassword2]").val();
			var newPwdMd5 = md5(newPayPassword);
			$("[name=newPayPassword2]").val(newPwdMd5);
		});
	});
</script>
</head>
	...
	<div id="header">
		<div class="layout">
			...
			<div id="nav" class="glb-nav">
				<ul class="clearfix">
					<li class="glb-nav-uc current"><a href="<%=basePath%>/transferController/to_enterprise_center"> <span></span>
					</a></li>
				...
				<div class="trade-tab">
					...
					<div class="trade-tab-bd">
						<form class="form-bd form-bd-trade"
							action="enterprise_money" id="companyForm" method="post">
									...
									<div class="form-field">
										<div class="form-twin-group">
											<input class="ipt ipt-money" type="text"
												name="winthdrawMoney2" id="Allmoney2" value=""
												placeholder="请输入提现金额" /> <label id="hint2"></label><br />
										</div>
										<input type="hidden" name="nameBalance2" id="nameBalance2"
											value="${sessionScope.enterpriseUser.balance }" /> <input
											type="hidden" name="" id="yhkh1"
											value="<%=request.getSession().getAttribute("yhkhEnterprise")%>" />
										<div class="form-twin-group">
											<span class="unit">元</span>
										</div>
										<div class="form-cb-group form-twin-group form-twin-rapid">
											<input type="checkbox" name="cbAllMoney2" class="cb" /> 全部提现
										</div>
							...
							<div class="form-item form-item-money">
								...
								<div class="form-entity">
									<div class="form-field">
										<div class="form-twin-group">
											<p class="form-text strong" id="paritiesEnterprise">0.00</p>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="form-item">
								...
								<div class="form-entity">
									<div class="form-field">
										<input id="pwd2" class="ipt ipt-pwd" type="password"
											name="payPassword2" value="" placeholder="请输入支付密码" /> <label
											id="hint1"></label><br />
									</div>
								</div>
							</div>
