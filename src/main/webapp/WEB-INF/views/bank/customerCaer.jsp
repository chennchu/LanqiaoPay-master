...
<body style="background: #e1e9eb;">
	...
	<div class="rightcon">
		<div class="right_con">
			...
			<ul class="glb-search">
				<!-- 通过用户的需求查到相关信息 -->
				<form action="customerServiceController/to_customerCaer"
							...
							<c:forEach items="${requestScope.customerList }" var="cust">
								<c:if test="${cust.state == 0 }">
									<tr>
										...
										<td>${cust.money }</td>
										<td><fmt:formatDate value="${cust.time }" type="both" /></td>
										<td><c:if test="${cust.operation==1 }">提现</c:if></td>
										<td>待审核</td>
										<td><a class="btn btn-primary"
											href="customerServiceController/to_withdrawalTrue?id=${cust.id }">确定通过审核</a>
										</td>
									</tr>
								</c:if>
							</c:forEach>
