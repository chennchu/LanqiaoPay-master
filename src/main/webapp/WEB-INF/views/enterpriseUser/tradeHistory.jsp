	...
	$(function(){
		...
		$("[name=submitForm]").click(function(){
			...
			if(tradeType == 3){//需要去查询转账记录
				alert(condations);
				this.href='${applicationScope.url }/enterpriseUser/enterpriseUserTransferHistory?'+condations;
			}
			else{//充值和提现
				alert(condations);
				this.href='${applicationScope.url }/enterpriseUser/enterpriseUserTradeHistory?'+condations;
			}
		});
...
$(function(){
	...
	kkpager.generPageHtml({
		...
		//链接前部
		hrefFormer : '${applicationScope.url }/enterpriseUser/enterpriseUserTradeHistory',
		//链接尾部
...
$(function(){
	...
	kkpager.generPageHtml({
		...
		totalRecords : totalRecords,
		//链接前部
		hrefFormer : '${applicationScope.url }/enterpriseUser/enterpriseUserTransferHistory',
		//链接尾部
		hrefLatter : '&tradeType=${userTradePage.tradeType}&tradeDateRangeType=${userTradePage.tradeDateRangeType}'
		+'&tradeNumber=${userTradePage.tradeNumber}&tradeName=${userTradePage.tradeName}'
		+'&begainTradeRange=<%=getBegainTradeRangeStr%>&endeTradeRange=<%=getEndeTradeRangeStr%>'
				...
				<div class="glb-list-box">
					<div class="glb-list-hd clearfix">
						...
						<div class="list-pay-side">
							<span>可用余额</span>
							<span>${requestScope.enterpriseUserBalance==null? 0:requestScope.enterpriseUserBalance}</span>
						</div>
					</div>
