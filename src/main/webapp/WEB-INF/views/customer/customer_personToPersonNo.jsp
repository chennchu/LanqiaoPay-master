...
<script type="text/javascript">
	...
	$(function() {
		...
		kkpager.generPageHtml({
			...
			//链接尾部
			hrefLatter : '/toCustomerTransfer_personToPerson',
			getLink : function(n) {
		...
		$("[name=orderSelect]").change(function(){
		    ...
		    $aa.attr('href','customerServiceController/toCustomerTransfer_personToPerson?orderSelect=' + order);    
		  	$aa.get(0).click();     
		});
	});
</script>
					...
					<c:forEach items="${requestScope.transferList }" var="transfer">
						<tr>
							...
							<td><a class="btn btn-primary"
									href="transferController/reviewTransferPTP?id=${transfer.id }">
									确定通过审核</a>
							</td>
						</tr>
					</c:forEach>
