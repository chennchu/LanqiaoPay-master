...
<script type="text/javascript">
	...
	$(function() {
		...
		kkpager.generPageHtml({
			...
			//链接尾部
			hrefLatter : '/enterpriseToEnterpriseNo',
			getLink : function(n) {
		...
		$("[name=orderSelect]").change(function(){
		    ...
		    $aa.attr('href','customerServiceController/toCustomerTransfer_enterpriseToEnterprise?orderSelect=' + order);    
		  	$aa.get(0).click();     
		});
	});
</script>
				...
				<tbody>
					<c:forEach items="${requestScope.transferList }" var="transfer">
						<tr>
							...
							<td>${transfer.toEnterprise.enterpriseName }</td>
							<td>${transfer.money }</td>
							<td><fmt:formatDate value="${transfer.time }" type="both" /></td>
							<td>等待审核</td>
							<td><a class="btn btn-primary"
									href="transferController/reviewTransferETE?id=${transfer.id }">
									确定通过审核</a>
							</td>
						</tr>
					</c:forEach>
